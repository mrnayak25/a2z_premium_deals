...
- Url: https://a2zpremiumdeals.com/viewproperty/${land.id}`;
  ...
  <div className="absolute top-0.2 left-4 z-10" data-aos="fade-down" data-aos-delay="400">
          <button className=" text-white bg-orange-500 rounded-lg p-0.5 px-2 font-medium">{index}</button>
        </div>
        <div className="absolute top-0.1 right-4 z-10" data-aos="fade-down" data-aos-delay="400">
          <RWebShare
            ...
            <button className="text-white bg-orange-500 rounded-lg p-2  font-medium  ">
              <img className="h-4 transition-transform duration-300 hover:scale-110" src={share} alt="share" />
            </button>
          </RWebShare>
        </div>
        {/* <div className="absolute top-36 right-9 z-10" data-aos="fade-down" data-aos-delay="200">
          <button className="text-white bg-green-500 rounded-lg p-1 px-2 font-medium">For {land.sellOrRent}</button>
        </div> */}
        <Link to={`/viewproperty/${land.id}`}>
    <img src={land.imageUrls[0]} alt="home" className="h-48 rounded-2xl" data-aos="fade-down" data-aos-delay="200"/>
    </Link>
  </div>
  ...
  <span className="text-sky-800 space-x-2"><i class="fa-solid fa-location-dot text-slate-900"></i><span>{land.location}</span></span><br/>
  {/* <span className="text-xs text-gray-500">{land.description}</span> */}
  </div></Link>

...
function Carousel() {
  ...
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev === lands.length - 1 ? 0 : prev + 1));
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev === 0 ? lands.length - 1 : prev - 1));
  };
    ...
    <div className="relative  m-3">
      <div className="relative h-56 overflow-hidden rounded-lg md:h-96">
        {lands.map((land, index) => (
          <div  key={land.id}
            ...
            <img src={land.imageUrl} className="block w-full h-full object-cover" alt={land.title} />
          </div>
        ))}
      </div>
      <div className="absolute z-30 flex space-x-3 bottom-5 left-1/2 transform -translate-x-1/2">
        {lands.map((_, index) => (
          <button  key={index}  type="button"
            ...
            onClick={() => setCurrentSlide(index)} aria-current={index === currentSlide} aria-label={`Slide ${index + 1}`}
          ></button>
        ))}
      </div>
      ...
      >
        <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 group-hover:bg-white/50 focus:ring-4 focus:outline-none">
          <svg  className="w-4 h-4 text-white group-hover:text-gray-800"  aria-hidden="true"
      ...
      <button  type="button"
        ...
        onClick={nextSlide}  >
        <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 group-hover:bg-white/50 focus:ring-4 focus:outline-none">
          <svg className="w-4 h-4 text-white group-hover:text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"  fill="none"  viewBox="0 0 6 10"
          >
